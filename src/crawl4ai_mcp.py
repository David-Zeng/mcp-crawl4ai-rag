...
async def process_local_file(
    ctx: Context, file_path: str, source_name: str = None, chunk_size: int = 5000
) -> str:
    """
    Read a local markdown or text file and store its content in Supabase.

    This tool processes local .md or .txt files by reading their content,
    chunking it appropriately, and storing it in the same embedding database
    as web-crawled content for unified RAG queries.

    Args:
        ctx: The MCP server provided context
        file_path: Path to the local .md or .txt file to process
    ...
    try:
        ...
        if not file_path_obj.suffix.lower() in [".md", ".txt"]:
            return json.dumps(
                {
                    "success": False,
                    "file_path": file_path,
                    "error": "File must have .md or .txt extension",
                },
                indent=2,
            )
        ...
        if not content.strip():
            return json.dumps(
                {"success": False, "file_path": file_path, "error": "File is empty"},
                indent=2,
            )

        # Chunk the content using the same function as web crawling
        ...
        source = source_name or file_path_obj.stem
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Extract metadata (same as web crawling)
            meta = extract_section_info(chunk)
            meta["chunk_index"] = i
            meta["url"] = file_url
            meta["source"] = source
            meta["file_type"] = file_path_obj.suffix[1:]  # Remove the dot
            meta["file_name"] = file_path_obj.name
        ...
        # Add to Supabase using the same function as web crawling
        add_documents_to_supabase(
            supabase_client,
            urls,
            chunk_numbers,
        ...
        return json.dumps(
            {"success": False, "file_path": file_path, "error": str(e)}, indent=2
        )
