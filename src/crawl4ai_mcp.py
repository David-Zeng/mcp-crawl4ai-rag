...
async def crawl_single_page(ctx: Context, url: str) -> str:
    ...
    try:
        ...
        if result.success and result.markdown:
            ...
            update_source_info(source_id, source_summary, total_word_count)

            # Add documentation chunks to the database (AFTER source exists)
            add_documents_to_db(
...
async def smart_crawl_url(
    ...
    """
    Intelligently crawl a URL based on its type and store content in the database.
    ...
    All crawled content is chunked and stored in the database for later retrieval and querying.
    ...
    try:
        ...
        # Process results and store in the database
        urls = []
        chunk_numbers = []
        contents = []
        metadatas = []
        ...
        # Add documentation chunks to the database (AFTER sources exist)
        batch_size = 20
        add_documents_to_db(
            urls,
            chunk_numbers,
...
async def get_available_sources(ctx: Context) -> str:
    ...
    """
    try:
        from utils import get_async_db_conn
        ...
        if use_hybrid_search:
            ...
            try:
                ...
                vector_results = await search_documents(
                    ...
                    * 2,  # Get double to have room for filtering
    ...
    try:
        ...
        if use_hybrid_search:
            ...
            results = await search_code_examples(
                query=query, match_count=match_count, filter_metadata=filter_metadata
            )
...
@mcp.tool()
async def insert_local_document(
    ctx: Context, file_path: str, document_type: str, source_id: Optional[str] = None
) -> str:
    """
    Inserts content from a local document into the RAG database or processes it for the knowledge graph.

    This tool allows you to ingest local files, either as general text documents for RAG queries
    or as code for potential knowledge graph integration (with current limitations).

    Args:
        ctx: The MCP server provided context.
        file_path: The absolute path to the local document file.
        document_type: The type of document to insert. Can be 'text' or 'code'.
        source_id: Optional. A custom source ID for the document. If not provided, it will be derived from the file path.
    ...
    try:
        ...
        if document_type == "text":
            # Process as a text document for RAG
            derived_source_id = source_id if source_id else Path(file_path).name

            chunks = smart_chunk_markdown(content)

            urls = [f"file://{file_path}"] * len(chunks)
            ...
            for i, chunk in enumerate(chunks):
                meta = extract_section_info(chunk)
                meta["chunk_index"] = i
                meta["url"] = f"file://{file_path}"
                meta["source"] = derived_source_id
                meta["document_type"] = "local_text"
                metadatas.append(meta)
                total_word_count += meta.get("word_count", 0)

            url_to_full_document = {f"file://{file_path}": content}

            source_summary = extract_source_summary(derived_source_id, content[:5000])
            update_source_info(derived_source_id, source_summary, total_word_count)
            add_documents_to_db(
                urls, chunk_numbers, contents, metadatas, url_to_full_document
            )
            ...
            return json.dumps(
                {
                    ...
                    "chunks_stored": len(chunks),
                    "total_word_count": total_word_count,
                    "message": "Local text document inserted into RAG database.",
                },
                indent=2,
            )

        elif document_type == "code":
            # Process as code for knowledge graph (with limitations)
            ...
            if not knowledge_graph_enabled:
                return json.dumps(
                    {
                        "success": False,
                        "error": "Knowledge graph functionality is disabled. Set USE_KNOWLEDGE_GRAPH=true in environment to use this feature for code.",
                    },
                    indent=2,
                )

            # Current knowledge graph implementation is repository-centric
            return json.dumps(
                {
                    "success": False,
                    "error": "Direct insertion of individual local code files into the knowledge graph is not supported.",
                    "details": "The knowledge graph is designed to parse entire GitHub repositories. To add code, please use the `parse_github_repository` tool with a GitHub repository URL (.git ending).",
                    "file_path": file_path,
                    "document_type": document_type,
                },
                indent=2,
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": f"Unsupported document_type: {document_type}. Must be 'text' or 'code'.",
                },
                indent=2,
            )
