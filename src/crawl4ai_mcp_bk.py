...
async def insert_local_document(
    ...
    """
    Inserts content from local documents into the RAG database or processes them for the knowledge graph.
    ...
    Args:
        ctx: The MCP server provided context.
        file_paths: A list of absolute paths to the local document files.
        document_type: The type of document to insert. Can be 'text' or 'code'.
        source_id: Optional. A custom source ID for the documents. If not provided, it will be derived from the file path.
    ...
    for file_path in file_paths:
        try:
            ...
            if document_type == "text":
                ...
                # Update source information FIRST (before inserting documents)
                source_summary = extract_source_summary(
                    derived_source_id, content[:5000]
                )  # Use first 5000 chars for summary
                ...
                if extract_code_examples:
                    ...
                    if code_blocks:
                        ...
                        for i, (block, summary) in enumerate(
                            ...
                            code_urls.append(f"file://{file_path}")
                            code_chunk_numbers.append(i)
                            code_examples.append(block["code"])
                            code_summaries.append(summary)
                ...
                # For code, currently only supports parsing GitHub repositories
                # This part remains largely the same, but now it's inside the loop
                # and handles individual file processing.
                # The original note about not supporting direct insertion of local code files
                # into the knowledge graph still applies. This tool is for RAG.
                results.append(
                    {
                        "success": False,
                        "file_path": file_path,
                        "error": "Direct insertion of individual local code files into the knowledge graph is not supported by this tool. Use parse_github_repository for GitHub repositories.",
                    }
                )
            else:
                results.append(
